...
const BreakfastScreen = () => {
  ...
  useEffect(() => {
    const fetchDishes = async () => {
      try {
        const response = await axios.get('http://192.168.165.203:5000/food/5/items'); // Update URL if needed
  ...
  const totalItemsInCart = Object.keys(itemCounts).reduce((acc, id) => {
    ...
    if (dish && dish.canteenId === 5 && itemCounts[id] > 0) {  // Assuming canteenId is part of the dish
      acc += itemCounts[id];
    }
    return acc;
  }, 0);
