...
export default function AdminDashboard({ navigation }) {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://192.168.37.203:5000/admin/items');
  ...
  const deleteItem = async (id) => {
    Alert.alert('Confirm Delete', 'Are you sure you want to delete this item?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`http://192.168.37.203:5000/admin/delete-food/${id}`);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.itemCard}>
      {item.imageUrl ? (
        <Image source={{ uri: `http://192.168.37.203:5000${item.imageUrl}` }} style={styles.itemImage} />
      ) : (
  ...
  return (
    <View style={styles.container}>
      ...
      <FAB style={styles.fab} icon="plus" onPress={() => navigation.navigate('AddItem')} />
    </View>
  );
}
