...
const Canteen4 = () => {
  ...
  useEffect(() => {
    const fetchDishes = async () => {
      try {
        const response = await axios.get('http://192.168.165.203:5000/food/4/items');
  ...
  const totalItemsInCart = Object.keys(itemCounts).reduce((acc, id) => {
    const dish = dishes.find((dish) => dish._id === id);  // Get the current dish by ID
    if (dish && dish.canteenId === 4 && itemCounts[id] > 0) {  // Assuming canteenId is part of the dish
      acc += itemCounts[id];
    }
    return acc;
  }, 0);
