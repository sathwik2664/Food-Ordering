...
export default function AdminDashboard5({ navigation }) {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://192.168.0.107:5000/admin/5/items');
  ...
  const deleteItem = async (id) => {
    Alert.alert('Confirm Delete', 'Are you sure you want to delete this item?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await axios.delete(`http://192.168.0.107:5000/admin/5/delete-food/${id}`);
  ...
  return (
    <View style={styles.container}>
      ...
      <FAB style={styles.fab} icon="plus" onPress={() => navigation.navigate('AddItem5')} />
    </View>
  );
}
