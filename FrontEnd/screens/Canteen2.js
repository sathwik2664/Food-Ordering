...
const Canteen2 = () => {
  ...
  const { canteenId = 2 } = route.params || {}; // Default to 2 if no canteenId provided
  ...
  const fetchDishes = useCallback(async () => {
    try {
      const response = await axios.get(`http://192.168.165.203:5000/food/${canteenId}/items`);
      if (response.status === 200 && Array.isArray(response.data.data)) {
        ...
        const savedCart = await AsyncStorage.getItem(`canteen${canteenId}_cart`);
        if (savedCart) {
  ...
  useEffect(() => {
    fetchDishes();
  }, [canteenId, fetchDishes]);

  useEffect(() => {
    const saveCart = async () => {
      try {
        await AsyncStorage.setItem(`canteen${canteenId}_cart`, JSON.stringify(itemCounts));
      } catch (error) {
  ...
  const viewCart = () => {
    ...
    navigation.navigate('Cart', { cart, canteenId }); // Pass dynamic canteenId
    console.log('Sent canteenId:', canteenId);
  };
